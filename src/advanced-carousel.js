(function () {
  ...
  function advancedCarousel($compile, $timeout, $ionicScrollDelegate) {
    ...
    function linkFunc(scope, el, attr, ctrl) {
      ctrl.el = el;
      angular.extend(ctrl.options, ctrl.carouselOptions);
      ...
      html += 'scrollbar-x="false" delegate-handle="' + ctrl.options.carouselId + '" ' +
        'zooming="false" direction="x" overflow-scroll="' + ctrl.options.overflowScroll + '" has-bouncing="' + ctrl.options.hasBouncing + '">' +
      ...
      function allowVerticalScroll() {
        ...
        sv.touchStart = function (e) {
          ...
          if (originaltouchMove) originaltouchStart.apply(sv, [e]);
        };
  ...
  function Controller($scope, $element, $timeout, $window, $ionicScrollDelegate) {
    ...
    function initCarousel() {
      ...
      function processNow() {
        ...
        if (vm.options.centerOnSelect) {
          ...
          vm.options.maxScroll = vm.navElem[0].offsetWidth - widthCarousel;

          marginRight = marginLeft = vm.options.maxScroll / 2;
    ...
    function onScroll() {
      ...
      if (pct > 1) {
        ...
        vm.pullrefreshIndicator.css({
          'transform': 'translateX(' + pctRotIndicator * -56 + 'px)'
        });
      }
    ...
    function onPullRefresh() {
      if (!vm.options.pullRefresh.loading) {
        ...
        vm.navElem.css({
          'width': vm.options.widthCarousel + 45 + 'px'
        });
    ...
    function onEndRefresh() {
      ...
      vm.navElem.css({
        'width': vm.options.widthCarousel + 'px'
      });
      $ionicScrollDelegate.$getByHandle(vm.options.carouselId).resize();
